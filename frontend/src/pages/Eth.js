...
let newsURL='https://feed.cryptoquote.io/api/v1/news/headlines?search=ETH&key=778fae00-359b-11eb-a7c8-83b5e7f8291c'
...
function Eth() {
  ...
  const [eEthereums, seteEthereum]=useState(null)
  const [eEthereumsNews, seteEthereumNews]=useState(null)
  ...
  let priceURL= 'https://rest-sandbox.coinapi.io/v1/ohlcv/GEMINI_SPOT_ETH_USD/latest?period_id='
...
//https://rest.coinapi.io/v1/ohlcv/GEMINI_SPOT_ETH_USD/history?period_id=1DAY&time_start=2016-01-01T00:00:00&time_end=2020-01-01T00:00:00
   ...
   //ETHRecoms array declaration
    ...
    //Iterate over ETHrecoms array to show new one
    recoms.forEach(recom=>{
      ETHrecoms[recom.crypto]=[...ETHrecoms[recom.crypto],recom]
    })
...
const menu = (
  ...
  return (
    ...
    <Dropdown overlay={menu}>
      ...
      >
        <VictoryLabel text="ETH/USD Price" x={225} y={30} textAnchor="middle"/>
      ...
      <VictoryLine
        ...
        data={eEthereums.map(b=>({x:b.time_close,y:b.price_close}))}
      />
    ...
    <Col span={10} style={{padding:"30px 10px 30px 0"}}>
      ...
      {/*-----eEthereums news list from most recent-----*/}
          {eEthereumsNews?
          <List
          itemLayout="vertical"
          size="small"
